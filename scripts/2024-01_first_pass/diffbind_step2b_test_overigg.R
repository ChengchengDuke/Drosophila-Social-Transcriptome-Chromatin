...
deseq_obj_file <- "/Volumes/ExtremePro/Emi/social_experience/cutandrun_dec22/differential_analysis/data_output/cutnrun_H3K27ac_promoter1kb_deseq_counts.Rds"
deseq_obj_igg_file <- "/Volumes/ExtremePro/Emi/social_experience/cutandrun_dec22/differential_analysis/data_output/cutnrun_IgG_promoter1kb_deseq_counts.Rds"


# Load deseq obj
deseq_obj <- readRDS(deseq_obj_file)
deseq_obj_igg <- readRDS(deseq_obj_igg_file)

deseq_obj <- cbind(deseq_obj,deseq_obj_igg)
...
colData(deseq_obj)$is_ctrl <- as.factor(ifelse(str_detect(colnames(deseq_obj),"IgG"),"igg","signal"))
colData(deseq_obj)$condition <- as.factor(paste(colData(deseq_obj)$housing, colData(deseq_obj)$is_ctrl,sep = "_"))

deseq_obj@design <- ~ 0 + condition
...
as.data.frame(res_list[[1]]) %>% ggplot(aes(baseMean, log2FoldChange, color = padj < 0.05)) + geom_point()
# Shrinkage
