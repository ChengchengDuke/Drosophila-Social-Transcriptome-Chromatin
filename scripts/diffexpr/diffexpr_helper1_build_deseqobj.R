...
rnaseq_batches <- rnaseq_batches %>%
  dplyr::select(rownames,dissection)
# Join both tables by sample name 
deseq_obj_coldata <- left_join(as.data.frame(colData(deseq_obj)), # Convert to regular data frame
                               rnaseq_batches, by = c("sample_name" = "rownames"))
# DSX samples have NA as dissection batch 
deseq_obj_coldata <- deseq_obj_coldata %>%
  mutate(dissection = ifelse(is.na(dissection),"cd",dissection)) %>%  # If dissection is NA, change that to cd
  mutate(dissection = ifelse(dissection == "ac","ac","notac")) # Convert to only ac or not ac
deseq_obj_coldata <- DataFrame(deseq_obj_coldata) # Convert back to deseq2-like dataframe 
rownames(deseq_obj_coldata) <- deseq_obj_coldata$sample_name # Add rownames to coldata 
colData(deseq_obj) <- deseq_obj_coldata # Update coldata

######
colData(deseq_obj)$housing <- as.factor(str_split(colnames(deseq_obj),pattern = "_",n = 3) # Split each name into three terms by "_"
                                        %>% map_chr(2)) # Get second element from each list-element, chr means convert output to character vector
colData(deseq_obj)$genotype <- as.factor(str_split(colnames(deseq_obj),pattern = "_",n = 3) %>% map_chr(1)) # As factor to make compatible with deseq2 design input
colData(deseq_obj)$condition <- as.factor(paste0(colData(deseq_obj)$genotype,"_",colData(deseq_obj)$housing)) # Paste housing and genotype 

### Filter out non mRNA genes
filt_out_rna_rownames <- rownames(deseq_obj) %>%
  str_split(":") %>% map_chr(1) # Split gene name by ":", get the first element

### Get the rna types that we want to filter out
filt_out_rna <- rownames(deseq_obj)[str_detect(rownames(deseq_obj),":")] %>% # detect rownames with ":", which are the rna genes
  str_split(":") %>% map_chr(1) 
filt_out_rna <- filt_out_rna %>%
  unique()
filt_out_rna <- filt_out_rna[!filt_out_rna %in% c("lncRNA","asRNA")] # Keep lncRNA and asRNA, might also include mt and His genes here
deseq_obj <- deseq_obj[!filt_out_rna_rownames %in% filt_out_rna,] # Filter the deseq_obj
...
# Save deseq_obj with combat counts
