...
# Test
# This part is specific to genotype 
contrast_list <- data.frame(contrast = "condition",
                            level1 = levels(colData(deseq_obj)$condition)[-c(1:2)], # removing the first 2 those are CS
                            level2 = rep(levels(colData(deseq_obj)$condition)[c(1:2)])) # get the first 2 those are CS, repeat it for every mutant 
