...
genotype_list <- as.list(levels(colData(deseq_obj)$genotype))



genotype_deseq_list <- map(genotype_list,function(.x, deseq_obj){
  deseq_obj[,colData(deseq_obj)$genotype == .x]
}, deseq_obj = deseq_obj)
...
# Test
res_list <- map(genotype_deseq_list, function(x) results(contrast = contrast, 
                                                    object = x,
                                                    alpha = 0.05) )
...
names(res_ashr_list) <- paste0(res_names,"_",unlist(genotype_list))
